...
public class TCPServer {
	...
	// TCP Socket 프로그래밍
	// TCP : 데이터 전달의 신뢰성을 최대한 보장하기 위한 연결 지향형 통신
	//		 순차적으로 데이터를 전달하고 확인하고 오류 시 재전송
	...
	public void serverStart() {
		//1. 서버의 포트번호 정함
		int port = 8500; // 포트번호는 0~65535 사이 지정 가능
		...
		// PrintWriter는 println() print() printf()등 다양한 출력 함수를 제공함으로써 파일 출력을 편하게 해준다. 
		...
		try {
			...
			// InputStreamReader : 바이트 기반 스트림과 문자 기반 스트림 연결에 사용하는 스트림
			
			pw = new PrintWriter(os); // PrintWriter는 별도의 연결용 스트림 필요 없다
			
			//7-1. 스트림을 통해 읽고 쓰기
			pw.println("[서버 접속 성공]");
			pw.flush(); // 스트림에 있는 내용을 모두 밀어냄
			// close()도 밀어내는 게 가능하지만 flush()를 사용하는 이유
			// ==> 스트림을 닫지 않은 상태에서 내용을 내보내고 싶은 경우가 있음
			
			//7-2. 클라이언트가 -> 서버에게 입력(메세지 전송 받기)
			String message = br.readLine(); // 클라이언트 메세지를 한 줗 읽어옴
			System.out.println(clientIP + "가 보낸 매세지 : " + message);
